...
const ShippingBoxesPage = () => {
  const entry = productData['shipping-boxes'];

  // Adapt EnrichedProductEntry to match ProductPageProps interface
  const adaptedData = {
    name: entry.name,
    description:
      (typeof entry.description === 'string' ? entry.description : undefined) || `Premium ${entry.name.toLowerCase()} solutions`,
    heroImage:
      (typeof entry.heroImage === 'string' ? entry.heroImage : undefined) || "products-box-img_x8vu4b",
    modelPath:
