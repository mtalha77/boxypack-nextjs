...
const ProductOverview: React.FC<ProductOverviewProps> = ({ productData }) => {
  ...
  // Ensure exactly 3 paragraphs - prioritize overview.paragraphs
  let overviewParagraphs: string[] = [];
  
  // First, try to use overview.paragraphs if available
  if (overview?.paragraphs && Array.isArray(overview.paragraphs) && overview.paragraphs.length > 0) {
    // Take first 3 paragraphs, ensure we have exactly 3
    overviewParagraphs = [...overview.paragraphs];
    
    // If we have less than 3, pad with description or empty
    while (overviewParagraphs.length < 3) {
      if (description && !overviewParagraphs.includes(description)) {
        overviewParagraphs.push(description);
      } else {
        overviewParagraphs.push('');
      }
    }
    
    // If we have more than 3, take only first 3
    overviewParagraphs = overviewParagraphs.slice(0, 3);
  } 
  // Fallback: if no overview paragraphs but we have description, split it
  else if (description) {
    // Split description into 3 meaningful parts
    const sentences = description.split(/[.!?]+/).filter(s => s.trim().length > 0);
    
    if (sentences.length >= 3) {
      // If we have 3+ sentences, distribute them
      ...
      // If less than 3 sentences, split by words
      const words = description.split(' ');
      const chunkSize = Math.ceil(words.length / 3);
      for (let i = 0; i < 3; i++) {
        const chunk = words.slice(i * chunkSize, (i + 1) * chunkSize).join(' ');
        if (chunk) overviewParagraphs.push(chunk);
      }
    }
  ...
  // Filter out empty paragraphs and ensure we have at least one valid paragraph
  const validParagraphs = overviewParagraphs.filter(p => p && p.trim().length > 0);

  if (validParagraphs.length === 0) {
    return null;
  }
  
  // Ensure we always have exactly 3 paragraphs for display (pad with empty if needed)
  while (overviewParagraphs.length < 3) {
    overviewParagraphs.push('');
  }

  return (
    <section className="py-24 bg-gradient-to-b from-[#f8f5f1] to-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="text-center space-y-6 mb-20">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-[3rem] font-semibold leading-tight text-[#171717] tracking-tight max-w-4xl mx-auto">
            {overviewTitle}
          </h2>
          <div className="w-24 h-1 bg-gradient-to-r from-[#0c6b76] to-[#0ca6c2] mx-auto rounded-full" />
        </div>

        {/* Three Paragraph Layout - Enhanced Card Design */}
